...
function displayTransactionsForCurrentMonth() {
  ...
  transactions.forEach((transaction, index) => {
    ...
    const categoryName =
      categoryMap[transaction.category] || "Categoria desconhecida";
  ...
  const footerTransactionCount = document.getElementById(
    "footerTransactionCount"
  );

  if (footerTotalAmount && footerTransactionCount) {
    ...
    footerTransactionCount.textContent = `${transactionCount} transaç${
      transactionCount !== 1 ? "ões" : "ão"
    }`;
  }
  // <-- FIM DO NOVO BLOCO -->
  ...
  if (valorDia01Element && valorDia15Element) {
    ...
    const valorAtual01 =
      parseFloat(
        valorDia01Element.textContent.replace(/[R$\s.]/g, "").replace(",", ".")
      ) || 0;
    const valorAtual15 =
      parseFloat(
        valorDia15Element.textContent.replace(/[R$\s.]/g, "").replace(",", ".")
      ) || 0;

    animateValue(valorDia01Element, valorAtual01, totalDia01, 800);
    animateValue(valorDia15Element, valorAtual15, totalDia15, 800);

    qtdDia01Element.textContent = `${countDia01} entrada${
      countDia01 === 1 ? "" : "s"
    }`;
    qtdDia15Element.textContent = `${countDia15} entrada${
      countDia15 === 1 ? "" : "s"
    }`;
  }
...
document
  .getElementById("type")
  .addEventListener("change", fetchPreviousDebtNames);
